...
function isRendered(domObj) {
    ...
    //100% text-indent
    if(domObj.parentNode == document.documentElement && textIndentProperty == getStyle(domObj.parentNode, "width") && overFlowProperty == "hidden" && getStyle(domObj,"white-space") == "nowrap")
    	return false;
...
function getNodeValue(node){
	if(node){
		node.nodeValue = node.nodeValue.replace(/\u00a0/g, " ");	// replace &nbsp; with a space
    ...
    for (var i in sheets) {
        ...
        for (var r in rules) {
            if (el.matches(rules[r].selectorText)) {
                ret.push("'css" + cnt + "':" + "'" + encodeURIComponent(rules[r].cssText) + "'");
                cnt++;
            }
        }
    }
...
if(heads && heads.length > 0){
  nodes.push([heads[0],0]);
}
nodes.push([document.documentElement,-1]);
...
var viewportEntry = "type:2,coord:[0,0,"+viewportWidth+","+viewportHeight+"]";
data += '{'+ viewportEntry.toString()+'},';
